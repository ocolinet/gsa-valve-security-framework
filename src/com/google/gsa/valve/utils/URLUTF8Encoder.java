...
URLUTF8Encoder {
    ...
    public static String encode(String s) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if ('A' <= ch && ch <= 'Z') { // 'A'..'Z'
                ...
                sbuf.append("%20");
            } else if // unreserved and others like / + (CLAZARO)
                (ch == '-' || ch == '_' || ch == '.' || ch == '!' || 
                 ch == '~' || ch == '*' || ch == '\'' || ch == '(' || 
                 ch == '+' || ch == '/' || ch == ')') {
                sbuf.append((char)ch);
    ...
    /**
     * This is the encoding method for java.net.URL classes
     * Important Note: if the URL contains an #, the URL class treats it as a 
     * reference (i.e. a link inside the page). For instance, the following path 
     * /pathwhatever#1/index.html will manage the following way
     * <ul>
     *     <li> Path: /pathwhatever </li>
     *     <li> Ref: #1/index.html </li>
     * </ul>
     * So if the URL contains # chars in the middle somehow, avoid to use this 
     * method and pass a String over instead
     * 
    ...
    /**
     * This is the encoding method for String-based Url
     * Important Note: if the URL contains an reference (internal page link)
     * the # char that defines the reference is encoded as well and the URL cannot 
     * be found then by the IIS webserver. 
     * For instance the following path /pathwhatever#1/index.html#reference 
     * will manage the following way:
     * <ul>
     *     <li> The first # is encoded: OK </li>
     *     <li> The second one (the reference) is encoded as well: NO OK </li>
     * </ul>
     * 
     * @param url Url chain
     * 
