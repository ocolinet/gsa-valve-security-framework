       ...
       if (this.sourceUrl.endsWith("/")){
        ...
        try {
        	...
        	int afterProcotol = sourceUrl.indexOf("//")+2;
                //workout for double "//"
                String strTail=sourceUrl.substring(afterProcotol,sourceUrl.length());
        	...
        	int lastSlash =sourceUrl.substring(afterProcotol).lastIndexOf("/");
        	...
        	if (lastSlash < 0) {
        		...
        		basehref = sourceUrl.substring(0, lastSlash + afterProcotol + 1);
        	}
        ...
        if (tag instanceof LinkTag) {
        	...
        	if (link != null) {
        		if (!link.startsWith("http")) {
        			try {
                                if (lsource.startsWith("mailto") ||lsource.startsWith("javascript") ) { 
                                    ((LinkTag) tag).setLink(lsource);

                                } else if (link.startsWith("/")) { 
            				String strBrowseUrl= hostHref + link;
            			        int afterProcotol = strBrowseUrl.indexOf("//")+2;
                                        String strTail=strBrowseUrl.substring(afterProcotol,strBrowseUrl.length());
                                        String strHead=strBrowseUrl.substring(0,afterProcotol);
            			        strTail = strTail.replaceAll("//","/");
                                        strBrowseUrl=strHead+strTail;
            			        ((LinkTag) tag).setLink(loginUrl + "?returnPath=" + strBrowseUrl);
            				logger.debug(sourceUrl + " (relative) rewritten URL: "+ loginUrl + "?returnPath=" + strBrowseUrl);
            				...
            				String strBrowseUrl= basehref + link;
                                        ...
                                        strBrowseUrl=strHead+strTail;
                                	((LinkTag) tag).setLink(loginUrl + "?returnPath=" + strBrowseUrl);
            			        logger.debug(sourceUrl + " (absolute)  URL: "+ loginUrl + "?returnPath=" + strBrowseUrl);
            			}
            		        			    			
        			} //catch (UnsupportedEncodingException e) {
        			catch (Exception e) {
						logger.debug ("Exception Visiting Tag: "+e.getMessage(), e);
					}        		
        		}else{
                            // Check is http://THIS REPOSITORY
                             if (link.startsWith(basehref)){
                                 ((LinkTag) tag).setLink(loginUrl + "?returnPath=" + link);
        	...
        	if (url != null) {
        		if (!url.startsWith("http")){
        			...
        			if (url.startsWith("/")) {         				
        				((ImageTag) tag).setImageURL(loginUrl + "?returnPath=" + hostHref + url);
        			 //   logger.debug(sourceUrl + " (img absolute)  URL: "+hostHref + url.substring(1));

        			} else { 
        				((ImageTag) tag).setImageURL(loginUrl + "?returnPath=" + basehref + url);  
        	...
        	// Protection
        	if ((url != null) && (url.startsWith("/"))) ((ScriptTag) tag).setAttribute("src", hostHref +"/"+url);
                   ...
                   if (link != null) {
                           if (!link.startsWith("http")) {
                                   try {
                                           ...
                                           strTail = strTail.replaceAll("//","/");
                                           strBrowseUrl=strHead+strTail;
                                           ((FrameTag) tag).setAttribute("src",loginUrl + "?returnPath=" + strBrowseUrl);
                                           ...
                                           logger.debug(sourceUrl + " (absolute)  URL: "+ loginUrl + "?returnPath=" + strBrowseUrl);
                                                    ...
                                                    logger.debug ("Exception at Frame Tag: "+e.getMessage(), e);
                                           }                       
                           }else{
                               // Check is http://THIS REPOSITORY
                                if (link.startsWith(basehref)){
                                    ((FrameTag) tag).setAttribute("src",loginUrl + "?returnPath=" + link);
                   ...
                   logger.debug( "SUNCHI " +tag.getTagName());
                   ...
                   if (link != null) {
                           if (!link.startsWith("http")) {
                                   try {
                                           ...
                                           strTail = strTail.replaceAll("//","/");
                                           strBrowseUrl=strHead+strTail;
                                            tag.setAttribute("src",loginUrl + "?returnPath=" + strBrowseUrl);
                                           ...
                                           logger.debug(sourceUrl + " (absolute)  URL: "+ loginUrl + "?returnPath=" + strBrowseUrl);
                                               ...
                                               logger.debug ("Exception at IFrame: "+e.getMessage(), e);
                                           }                       
                           }else{
                               // Check is http://THIS REPOSITORY
                                if (link.startsWith(basehref)){
                                    tag.setAttribute("src",loginUrl + "?returnPath=" + link);
                                }else {
                   ...
                   if (link != null) {
                           if (!link.startsWith("http")) {
                                   try {
                                           ...
                                           strTail = strTail.replaceAll("//","/");
                                           strBrowseUrl=strHead+strTail;
                                            tag.setAttribute("href",loginUrl + "?returnPath=" + strBrowseUrl);
                                           ...
                                           logger.debug(sourceUrl + " (absolute)  URL: "+ loginUrl + "?returnPath=" + strBrowseUrl);
                                               ...
                                               logger.debug ("Exception at Link: "+e.getMessage(), e);
                                           }                       
                           }else{
                               // Check is http://THIS REPOSITORY
                                if (link.startsWith(basehref)){
                                    tag.setAttribute("href",loginUrl + "?returnPath=" + link);
                                }else {
